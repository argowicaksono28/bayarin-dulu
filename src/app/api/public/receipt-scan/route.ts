...
// Public receipt scan endpoint — used by demo mode (no auth required).
// Identical logic to /api/receipt-scan but without the Supabase auth check.
...
If a field cannot be determined, use 0 or empty string. Always output valid raw JSON.`,
  ...
  console.log("[public/receipt-scan] calling Anthropic, file size:", bytes.byteLength)
